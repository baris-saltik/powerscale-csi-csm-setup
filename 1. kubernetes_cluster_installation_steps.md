...
### 5. Verify the init system (initd or SystemV). for all the nodes. Accordingly, the kubelet configuration is going to be modified soon. If it is systemd then cgroup driver should be systemd. There are two possibilities; cgroupfs and systemd for cgroup driver.
